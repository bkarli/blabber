...
async fn relay_never_decrypts_but_still_propagates_ciphertext() -> Result<()> {
    ...
    // the relay joins via a RelayInvite: read-only, no space key, no author
    let relay_invite = space_a.create_relay_invite().await?;
    let space_relay = node_relay.join_space_relay(relay_invite).await?;

    assert!(space_relay.author().is_none(), "a relay-joined space must have no author");
    ...
    // the relay never learns space content
    let relay_members = space_relay.list_members(&blobs_relay).await?;
    assert!(relay_members.is_empty(), "a blind relay must never decode member records");
