//! Unit-level check of the "blind relay" contract from the crypto/schema
//! redesign: a `Room`/`CallRoom` constructed with `key: None` must never
//! decrypt anything, and must never be able to originate content. This is
//! independent of the invite/join plumbing (that's exercised end-to-end by
//! relay_join.rs) - here we just directly construct the `None`-key objects
//! on a second node, against a doc that already has encrypted content in
//! it, and assert the read paths come back empty rather than erroring or
//! panicking, and the write paths refuse.
...
async fn room_with_no_key_reads_nothing_and_cannot_send() -> Result<()> {
    ...
    // a second node imports the same doc read-only, with no space key at all -
